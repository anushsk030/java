...
class NumberManager{
	...
	public synchronized void printEvenNumbersSquare() throws InterruptedException {
		while(!numberGenerated||generatedNumber%2!=0) {
			wait();
		}
		//for(int i=2;i<generatedNumber;i+=2) {
			System.out.print("Square of  "+generatedNumber+": "+generatedNumber+"^2"+"="+generatedNumber*generatedNumber+" ");
		//}
	...
	public synchronized void printOddNumbersCube() throws InterruptedException{
		
			while(!numberGenerated||generatedNumber%2==0) {
				wait();
			}
			//for(int i=1;i<generatedNumber;i+=2) {
				System.out.print("Cube Of "+generatedNumber+": " +generatedNumber+"^3"+"="+generatedNumber*generatedNumber*generatedNumber+" ");
			//}
			numberGenerated=false;
...
class EvenPrinterThread extends Thread{
	...
	public void run() {
		while(true) {  //for infinite loop
			try {
				numberManager.printEvenNumbersSquare();
			} catch (InterruptedException e) {
...
class OddPrinterThread extends Thread{
	...
	public void run() {
		while(true) {  //for infinite loop
			try {
				numberManager.printOddNumbersCube();
			} catch (InterruptedException e) {
